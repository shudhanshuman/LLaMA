...
class DataLoader:
    ...
    def __init__(self, B, T, encoder):
        ...
        total_tokens = (len(self.tokens) // (B * T)) * (B * T)
        self.tokens = self.tokens[:total_tokens]
        self.num_batches = len(self.tokens) // (B * T)
        self.batches = self.tokens.view(self.num_batches, B, T)

        print(f"dataset has {len(self.tokens)} tokens")
        print(f"dataset has {self.num_batches} batches")

        self.ptr = 0  

    def get_batch(self):
        x = self.batches[self.ptr]
...
data = DataLoader(B, T , encoder=enc)
...
for i in range(max_steps):
    ...
    tokens_per_sec = (data.B * data.T * accumulation_steps) / (t1 - t0)

    print(f"step {i:4d} | loss {loss_accum.item():.4f} | lr {lr:.4e} | time {dt:.2f}ms | tok/sec {tokens_per_sec:.2f}")
    logs.append(f"{i},{loss_accum.item():.4f},{lr:.6e},{dt:.2f},{tokens_per_sec:.2f}\n")
